...
pub async fn get_index_summary(configuration: &configuration::Configuration, ) -> Result<models::IndexSummaryBean, Error<GetIndexSummaryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IndexSummaryBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IndexSummaryBean`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetIndexSummaryError> = serde_json::from_str(&content).ok();
