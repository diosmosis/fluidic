...
pub async fn release(configuration: &configuration::Configuration, body: Option<&str>) -> Result<(), Error<ReleaseError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body = body;

    let uri_str = format!("{}/auth/1/websudo", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<ReleaseError> = serde_json::from_str(&content).ok();
