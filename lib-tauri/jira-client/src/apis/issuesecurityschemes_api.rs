...
pub async fn get_issue_security_scheme(configuration: &configuration::Configuration, id: &str) -> Result<models::SecuritySchemeJsonBean, Error<GetIssueSecuritySchemeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/issuesecurityschemes/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
...
pub async fn get_issue_security_schemes(configuration: &configuration::Configuration, ) -> Result<models::SecuritySchemesJsonBean, Error<GetIssueSecuritySchemesError>> {

    let uri_str = format!("{}/api/2/issuesecurityschemes", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SecuritySchemesJsonBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SecuritySchemesJsonBean`")))),
        }
    } else {
        let content = resp.text().await?;
