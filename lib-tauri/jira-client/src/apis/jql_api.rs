...
pub async fn get_auto_complete(configuration: &configuration::Configuration, ) -> Result<models::AutoCompleteResponseBean, Error<GetAutoCompleteError>> {

    let uri_str = format!("{}/api/2/jql/autocompletedata", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AutoCompleteResponseBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AutoCompleteResponseBean`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_field_auto_complete_for_query_string(configuration: &configuration::Configuration, predicate_value: Option<&str>, predicate_name: Option<&str>, field_name: Option<&str>, field_value: Option<&str>) -> Result<serde_json::Value, Error<GetFieldAutoCompleteForQueryStringError>> {
    ...
    let uri_str = format!("{}/api/2/jql/autocompletedata/suggestions", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
