...
pub async fn find_groups(configuration: &configuration::Configuration, max_results: Option<&str>, query: Option<&str>, exclude: Option<&str>, user_name: Option<&str>) -> Result<models::GroupSuggestionsBean, Error<FindGroupsError>> {
    ...
    let uri_str = format!("{}/api/2/groups/picker", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GroupSuggestionsBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GroupSuggestionsBean`")))),
        }
    } else {
        let content = resp.text().await?;
