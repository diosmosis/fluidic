...
/// struct for typed errors of method [`get_priorities1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPriorities1Error {
    Status401(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_priorities(configuration: &configuration::Configuration, ) -> Result<models::PriorityJsonBean, Error<GetPrioritiesError>> {

    let uri_str = format!("{}/api/2/priority", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PriorityJsonBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PriorityJsonBean`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_priorities1(configuration: &configuration::Configuration, max_results: Option<i32>, query: Option<&str>, project_ids: Option<Vec<i64>>, start_at: Option<i64>) -> Result<models::PriorityJsonBean, Error<GetPriorities1Error>> {
    ...
    let uri_str = format!("{}/api/2/priority/page", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetPriorities1Error> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns an issue priority
pub async fn get_priority(configuration: &configuration::Configuration, id: &str) -> Result<models::PriorityJsonBean, Error<GetPriorityError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/priority/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
