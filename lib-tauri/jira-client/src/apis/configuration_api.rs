...
/// struct for typed errors of method [`get_configuration1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetConfiguration1Error {
    Status401(),
...
pub async fn get_configuration1(configuration: &configuration::Configuration, ) -> Result<models::ConfigurationBean, Error<GetConfiguration1Error>> {

    let uri_str = format!("{}/api/2/configuration", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ConfigurationBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ConfigurationBean`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetConfiguration1Error> = serde_json::from_str(&content).ok();
