...
pub async fn get_custom_field_option(configuration: &configuration::Configuration, id: &str) -> Result<models::CustomFieldOptionBean, Error<GetCustomFieldOptionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/customFieldOption/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CustomFieldOptionBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CustomFieldOptionBean`")))),
        }
    } else {
        let content = resp.text().await?;
