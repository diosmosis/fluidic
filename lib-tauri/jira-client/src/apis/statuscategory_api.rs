...
pub async fn get_status_categories(configuration: &configuration::Configuration, request: Option<&str>, uri_info: Option<&str>) -> Result<models::StatusCategoryJsonBean, Error<GetStatusCategoriesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_request = request;
    let p_uri_info = uri_info;

    let uri_str = format!("{}/api/2/statuscategory", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = p_request {
        req_builder = req_builder.query(&[("request", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_uri_info {
        req_builder = req_builder.query(&[("uriInfo", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::StatusCategoryJsonBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::StatusCategoryJsonBean`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_status_category(configuration: &configuration::Configuration, id_or_key: &str) -> Result<models::StatusCategoryJsonBean, Error<GetStatusCategoryError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id_or_key = id_or_key;

    let uri_str = format!("{}/api/2/statuscategory/{idOrKey}", configuration.base_path, idOrKey=crate::apis::urlencode(p_id_or_key));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
