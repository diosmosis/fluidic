...
/// struct for typed errors of method [`expand_for_humans`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ExpandForHumansError {
    Status403(),
    Status404(),
    Status409(),
...
/// struct for typed errors of method [`expand_for_machines`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ExpandForMachinesError {
    Status403(),
    Status404(),
    Status409(),
    UnknownValue(serde_json::Value),
...
pub async fn expand_for_humans(configuration: &configuration::Configuration, id: &str) -> Result<models::HumanReadableArchive, Error<ExpandForHumansError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/attachment/{id}/expand/human", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::HumanReadableArchive`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::HumanReadableArchive`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ExpandForHumansError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Tries to expand an attachment. Output is raw and should be backwards-compatible through the course of time.
pub async fn expand_for_machines(configuration: &configuration::Configuration, id: &str) -> Result<models::AttachmentArchiveImpl, Error<ExpandForMachinesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/attachment/{id}/expand/raw", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ExpandForMachinesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns the meta-data for an attachment, including the URI of the actual attached file.
pub async fn get_attachment(configuration: &configuration::Configuration, id: &str) -> Result<serde_json::Value, Error<GetAttachmentError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/attachment/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
...
pub async fn get_attachment_meta(configuration: &configuration::Configuration, ) -> Result<models::AttachmentMetaBean, Error<GetAttachmentMetaError>> {

    let uri_str = format!("{}/api/2/attachment/meta", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AttachmentMetaBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AttachmentMetaBean`")))),
        }
    } else {
        let content = resp.text().await?;
