...
/// struct for typed errors of method [`get_upgrade_result`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUpgradeResultError {
    Status404(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`run_upgrades_now`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RunUpgradesNowError {
    Status403(),
    Status409(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_upgrade_result(configuration: &configuration::Configuration, ) -> Result<models::UpgradeResultBean, Error<GetUpgradeResultError>> {

    let uri_str = format!("{}/api/2/upgrade", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::UpgradeResultBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::UpgradeResultBean`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetUpgradeResultError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Runs any pending delayed upgrade tasks. Need Admin permissions to do this.
pub async fn run_upgrades_now(configuration: &configuration::Configuration, ) -> Result<(), Error<RunUpgradesNowError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RunUpgradesNowError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
