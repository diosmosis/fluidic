...
/// struct for typed errors of method [`set_property_via_restful_table`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SetPropertyViaRestfulTableError {
    Status403(),
    Status404(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_advanced_settings(configuration: &configuration::Configuration, ) -> Result<models::Property, Error<GetAdvancedSettingsError>> {

    let uri_str = format!("{}/api/2/application-properties/advanced-settings", configuration.base_path);
...
pub async fn get_property(configuration: &configuration::Configuration, permission_level: &str, key: &str, key_filter: Option<&str>) -> Result<models::Property, Error<GetPropertyError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_permission_level = permission_level;
    let p_key = key;
    let p_key_filter = key_filter;

    let uri_str = format!("{}/api/2/application-properties", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    req_builder = req_builder.query(&[("permissionLevel", &p_permission_level.to_string())]);
...
pub async fn set_property_via_restful_table(configuration: &configuration::Configuration, id: &str) -> Result<models::Property, Error<SetPropertyViaRestfulTableError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/application-properties/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SetPropertyViaRestfulTableError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
