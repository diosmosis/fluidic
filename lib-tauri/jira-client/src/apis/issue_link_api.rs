...
pub async fn delete_issue_link(configuration: &configuration::Configuration, link_id: &str) -> Result<(), Error<DeleteIssueLinkError>> {
    ...
    let uri_str = format!("{}/api/2/issueLink/{linkId}", configuration.base_path, linkId=crate::apis::urlencode(p_link_id));
...
pub async fn get_issue_link(configuration: &configuration::Configuration, link_id: &str) -> Result<models::IssueLinks, Error<GetIssueLinkError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_link_id = link_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IssueLinks`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IssueLinks`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn link_issues(configuration: &configuration::Configuration, link_issue_request_json_bean: models::LinkIssueRequestJsonBean) -> Result<(), Error<LinkIssuesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_link_issue_request_json_bean = link_issue_request_json_bean;

    let uri_str = format!("{}/api/2/issueLink", configuration.base_path);
