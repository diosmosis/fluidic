...
/// struct for typed errors of method [`set_base_url`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SetBaseUrlError {
    Status400(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`set_issue_navigator_default_columns_form`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SetIssueNavigatorDefaultColumnsFormError {
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_issue_navigator_default_columns(configuration: &configuration::Configuration, ) -> Result<serde_json::Value, Error<GetIssueNavigatorDefaultColumnsError>> {

    let uri_str = format!("{}/api/2/settings/columns", configuration.base_path);
...
pub async fn set_base_url(configuration: &configuration::Configuration, body: Option<&str>) -> Result<(), Error<SetBaseUrlError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body = body;

    let uri_str = format!("{}/api/2/settings/baseUrl", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SetBaseUrlError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Sets the default system columns for issue navigator. Admin permission will be required.
pub async fn set_issue_navigator_default_columns_form(configuration: &configuration::Configuration, columns: Option<Vec<String>>) -> Result<(), Error<SetIssueNavigatorDefaultColumnsFormError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SetIssueNavigatorDefaultColumnsFormError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
