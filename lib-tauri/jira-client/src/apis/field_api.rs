...
pub async fn create_custom_field(configuration: &configuration::Configuration, custom_field_definition_json_bean: Option<models::CustomFieldDefinitionJsonBean>) -> Result<models::FieldBean, Error<CreateCustomFieldError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_custom_field_definition_json_bean = custom_field_definition_json_bean;

    let uri_str = format!("{}/api/2/field", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FieldBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FieldBean`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_fields(configuration: &configuration::Configuration, ) -> Result<models::FieldBean, Error<GetFieldsError>> {
