...
/// struct for typed errors of method [`get_paginated_resolutions`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPaginatedResolutionsError {
...
pub async fn get_paginated_resolutions(configuration: &configuration::Configuration, max_results: Option<i32>, query: Option<&str>, start_at: Option<i64>) -> Result<serde_json::Value, Error<GetPaginatedResolutionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_max_results = max_results;
    let p_query = query;
    let p_start_at = start_at;

    let uri_str = format!("{}/api/2/resolution/page", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetPaginatedResolutionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Returns a resolution.
pub async fn get_resolution(configuration: &configuration::Configuration, id: &str) -> Result<models::ResolutionJsonBean, Error<GetResolutionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/api/2/resolution/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ResolutionJsonBean`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ResolutionJsonBean`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_resolutions(configuration: &configuration::Configuration, ) -> Result<models::ResolutionJsonBean, Error<GetResolutionsError>> {

    let uri_str = format!("{}/api/2/resolution", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
